...
# Permutation distribution for adjusted R²
perm_adjR2 <- function(y, X, n_perm, parallel = TRUE, n_cores = 1) {
  ...
  if (!parallel) {
    vapply(seq_len(n_perm), worker, numeric(1))
  } else if (.Platform$OS.type == "unix") {
    unlist(parallel::mclapply(seq_len(n_perm), worker, mc.cores = n_cores))
  } else {
    cl <- parallel::makeCluster(n_cores)
    on.exit(parallel::stopCluster(cl), add = TRUE)
    unlist(parallel::parLapply(cl, seq_len(n_perm), worker))
  }
}


# ---- Core regression + permutation function ----
run_analysis <- function(df, dataset_name, roi_idx, suffix) {
  ...
  fname <- file.path(
    output_dir,
    sprintf("regression_results_allPC_permutations_%s_%s.csv", suffix, dataset_name)
  )
...
for (f in files) {
  ...
  message("\n----------------------------------------------------")
  message("Processing dataset: ", basename(f))
  ...
  if (have_covs && length(roi_all_idx) > 0) {
  ...
  # ---- Run Cortical + Subcortical Analyses ----
  run_analysis(data_std, dataset_name, roi_cortical_idx,    "cortical")
  run_analysis(data_std, dataset_name, roi_subcortical_idx, "subcortical")
}
